...
local config={
    ...
    processor=function(name,value,row,node)
        ...
        elseif name=='Actual Startup Time' or name=='Actual Total Time' or name=='Total Runtime' then
        ...
        elseif name=="CTE Name" or name=="Subplan Name" then
            if node['Index Name'] or node['Relation Name'] then 
                return ''
            else
                return '{'..value..'}'
            end
    ...
    columns={{{"Parent Relationship","Parallel Aware","Join Type", "Node Type"},"Plan|Operation"},'|',
              {{"Relation Name","Index Name","CTE Name","Subplan Name"},"Object|Name"},'|',
              {"Alias","Obj|Alias"},'|',
...
function xplan.before_db_exec(obj)
    ...
    if xplan.autotrace=='off' or is_internal or not sql then return end
