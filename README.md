...
* Provides graph charts of the database performance, refer to the "chart" command
...
Download
-----------------------
DBCLI does not require compilation or installation, but depends on JRE 1.8+. Click `Download ZIP`, extract it and use it. Or you can also download from the published releases<br/>
Branch `With-JRE` contains the shrinked JRE bundle, if you don't have JRE installed,
you may switch to that branch to download. Due to regional and charset differences, this JRE bundle is not guaranteed to work well.<br/>
...
* In the `Layout` tab, set the buffer width and buffer height to `500+`
* In the `Font` tab, change the font size to a smaller value.

### Linux: Pre-Requisites
Linux must be the `64-bit` version, and ensure `glibc >= v2.4`(use `ldd --version` to verify) is installed. You can use command `<command> | more` to vertically/horizontally scroll the command output, or `more last` or scroll the last command output.

### OSX: Pre-Requisites
Only tested on OSX 10.12, lower versions should be fine. Java 1.8+ is required. It's recommended to add entry `<hostname>.local` into `/etc/hosts` to improve JVM performance.
For Example:
```
...
Of which `TNS_ADM` is optional unless you need to connect to Oracle via tnsnames.ora, or you may also refer to `init_sample.cfg` for more settings. An example is:
...
To permanently change the preferred platform from `Oracle` to another platform, run `set -p platform <platform>` after launching the console.
For example, `set -p platform db2`

Without the `-p` option, the change only takes effect in the current console window.
...
* **Public Commands: visible in all database platforms**
  ...
  * _Core_
    ...
    * `alias` : A command set, similar to the Unix `alias` command, mainly used to define the shortcut of other command lines, the definition is automatically stored in folder `aliases` for permanent reuse|
    * `history/his` : Check or run historical commands, and the historical commands are only kept in memory and available within the current window
  ...
  * _Screen_
    ...
    * `prompt/pro/echo` :Print text, if the text contains variables, then also convert as the the variable values, the command name can be different regarding to different db platforms
  * _Database_
    ...
    * `login/logon` :A command set, view or logon to a connection history, when a connection is successfully established, the logon info would be automatically stored in `data\password.dat`, so that user doesn't have to manually input the a/c for each time
  ...
  * _Scripting_
    * `*` :A command set, run customized script file which includes a batch of commands that supported in DBCLI, supports the features of variables, options and pre-compiled conditions. The engine maps the input parameters as `V1 - V20` variables except the parameter in `-<option>` format. This is the interface for some other commands to implement or extend, such as `snap`/`shell`/`ora`/`sys`/`chart`/`sql`/etc, different command owns different folder to store the script files, and those file names are recognized as the sub-command names.
  ...
  * _Variable_
    ...
    * `accept/acc` :Assigns user-input or file to a new variable
  ...
  * _Shell_
    ...
    * `shell` :A command set, run in remote SSH server with local shell/python/perl script(to be enhanced), this command is inherited from the scripting modules
  * _Internal_
...
* **Oracle Database**
  ...
  * _Report_
    * `awrdump` :Generate AWR report regarding the input time range or snap_id
    * `awrdiff` :Generate AWR diff report regarding the input time range or snap_id
    * `addmdump` :Generate ADDM report regarding the input time range or snap_id
    * `addmdiff` :Generate ADDM diff report regarding the input time range or snap_id
    * `ashdump` :Generate ASH report regarding the input time range or snap_id
  * _Extraction_
  ...
  * _Scripting_
    * `ora` :A command set that implements the scripting engine, to run predefined or customized script that used for DBA or db developer, this command has pre-defined many useful scripts
    * `sys` :A command set that inherited from `ora` and similar to `ora`, except can only be used by SYSDBA
    * `show` :A command set that inherited from `ora` and similar to `ora`, except that it doesn't accept input parameters
    * `exa` : A command set that inherited from `ora`, used to query `Exadata` cell configurations and statistics
  * _SQL*Plus_
    * `sqlplus` :Launch `sqlplus` console with the same login in native mode or embedded mode, plus other input parameters
    * `@` or `sp` :Run sqlplus script in embedded mode with the same login
  * _Settings_
    * `set instance` :Auto-limit the instance regarding any SQL that references the `gv$/x$/dba_hist` views
    * `set container` :Auto-limit the container ID regarding any SQL that references the views that have the `con_id` field
...
* **MySQL Database(to be enhanced)**
  ...
  * _Scripting_
    * `sql` :Inherited from the scripting engine, to run the script which contains a batch of all available commands
...
* **PostgreSQL Database(to be enhanced)**
  ...
  * _Scripting_
    * `sql` :Inherited from the scripting engine, to run scripts which contains a batch of all available commands
  * _Others_
...
* DDL commands that related to creating source code(`create trigger/package/function/procedure/type`) are cross-lines commands and should be ended with `/` in a stand-alone line. `create table/index/etc` are not of this kind because they are related to structures.
...
Line Wrap
-----------------------
For the output lines that are wider than the buffer width(in Posix System it's the screen width), the overflow part will be trimmed.
...
Color Solutions
-----------------------
`dbcli` has defined some colors that are used in different scenarios, those colors can be found by `set color`.<br/>
...
It's possible to turn off using colors, which is done by executing `set ansicolor off`

Releases
-----------------------
There are 5 zip files in the `releases` page that are used in different scenarios, all those releases don't include the `src` directory:
...
Besides, some SQL scripts come from the internet.
...
Similar to SQL*Plus script, a sub-command script is the combination of one or more existing commands that support user-input parameters.
...
The utility has created some pre-defined commands, if you want to modify those commands without concerning about overriding them by the new version, just create a sub-folder under the `ora` directory, and put the updated file into it, because for the scripts with same name, the one in the sub directory will be treated as higher priority. Or you may also use `ora -l <path>` to link to another work dir.
...
Different from sub-commands, the root command must be a lua script. To plug a user-defined lua script into the utility, just add its path in `data\plugin.cfg`, you may refer to `data\plugin_sample.cfg` for the example.
