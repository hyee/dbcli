...
COL "MAX|MEM,MAX|Go,0 Min|MEM,0 Min|Go,1 Min|MEM,1 Min|Go,2 Min|MEM,2 Min|Go,3 Min|MEM,3 Min|Go,4 Min|MEM,4 Min|Go,5 Min|MEM,5 Min|Go,6 Min|MEM,6 Min|Go,7 Min|MEM,7 Min|Go,8 Min|MEM,8 Min|Go,9 Min|MEM,9 Min|Go" FOR KMG2

SELECT inst, job, 
       '|' `|`,  
       MAX(IF(t='node',v,NULL)) `MAX|MEM`, 
       MAX(IF(t='gc',v,NULL)) `MAX|Go`,
       '|' `|`,
       AVG(IF(t='node' AND ts=DATE_FORMAT(now(),'%m%d-%H:%i'),v,null)) `0 Min|MEM`, 
       AVG(IF(t='gc' AND ts=DATE_FORMAT(now(),'%m%d-%H:%i'),v,null)) `0 Min|Go`,
       '|' `|`,
       AVG(IF(t='node' AND ts=DATE_FORMAT(date_add(now(), interval -1 minute),'%m%d-%H:%i'),v,null)) `1 Min|MEM`, 
       AVG(IF(t='gc' AND ts=DATE_FORMAT(date_add(now(), interval -1 minute),'%m%d-%H:%i'),v,null)) `1 Min|Go`,
       '|' `|`,
       AVG(IF(t='node' AND ts=DATE_FORMAT(date_add(now(), interval -2 minute),'%m%d-%H:%i'),v,null)) `2 Min|MEM`, 
       AVG(IF(t='gc' AND ts=DATE_FORMAT(date_add(now(), interval -2 minute),'%m%d-%H:%i'),v,null)) `2 Min|Go`,
       '|' `|`,
       AVG(IF(t='node' AND ts=DATE_FORMAT(date_add(now(), interval -3 minute),'%m%d-%H:%i'),v,null)) `3 Min|MEM`, 
       AVG(IF(t='gc' AND ts=DATE_FORMAT(date_add(now(), interval -3 minute),'%m%d-%H:%i'),v,null)) `3 Min|Go`,
       '|' `|`,
       AVG(IF(t='node' AND ts=DATE_FORMAT(date_add(now(), interval -4 minute),'%m%d-%H:%i'),v,null)) `4 Min|MEM`, 
       AVG(IF(t='gc' AND ts=DATE_FORMAT(date_add(now(), interval -4 minute),'%m%d-%H:%i'),v,null)) `4 Min|Go`,
       '|' `|`,
       AVG(IF(t='node' AND ts=DATE_FORMAT(date_add(now(), interval -5 minute),'%m%d-%H:%i'),v,null)) `5 Min|MEM`, 
       AVG(IF(t='gc' AND ts=DATE_FORMAT(date_add(now(), interval -5 minute),'%m%d-%H:%i'),v,null)) `5 Min|Go`,
       '|' `|`,
       AVG(IF(t='node' AND ts=DATE_FORMAT(date_add(now(), interval -6 minute),'%m%d-%H:%i'),v,null)) `6 Min|MEM`, 
       AVG(IF(t='gc' AND ts=DATE_FORMAT(date_add(now(), interval -6 minute),'%m%d-%H:%i'),v,null)) `6 Min|Go`,
       '|' `|`,
       AVG(IF(t='node' AND ts=DATE_FORMAT(date_add(now(), interval -7 minute),'%m%d-%H:%i'),v,null)) `7 Min|MEM`, 
       AVG(IF(t='gc' AND ts=DATE_FORMAT(date_add(now(), interval -7 minute),'%m%d-%H:%i'),v,null)) `7 Min|Go`,
       '|' `|`,
       AVG(IF(t='node' AND ts=DATE_FORMAT(date_add(now(), interval -8 minute),'%m%d-%H:%i'),v,null)) `8 Min|MEM`, 
       AVG(IF(t='gc' AND ts=DATE_FORMAT(date_add(now(), interval -8 minute),'%m%d-%H:%i'),v,null)) `8 Min|Go`,
       '|' `|`,
       AVG(IF(t='node' AND ts=DATE_FORMAT(date_add(now(), interval -9 minute),'%m%d-%H:%i'),v,null)) `9 Min|MEM`, 
       AVG(IF(t='gc' AND ts=DATE_FORMAT(date_add(now(), interval -9 minute),'%m%d-%H:%i'),v,null)) `9 Min|Go`
FROM   (SELECT t, INSTANCE inst, date_format(TIME, '%m%d-%H:%i') ts, job, ROUND(AVG(VALUE),4) v
        FROM   (SELECT 'gc' t, a.*
