/*[[List Top SQLs. Usage: @@NAME [{<yymmddhh24mi> [<yymmddhh24mi>} | {-sql <partial digest>} | -f"<filter>" | -u ] [-local]
    ...
    --[[--
        ...
        &tail: {
            default={any_value(substr(replace(replace(replace(replace(trim(Query),'\n',' '),' ','<>'),'><',''),'<>',' '),1,150)) sql_text}
...
COL "Write_Keys,Cop|Keys,Cop|Tasks" FOR TMB2
...
SELECT &hour &grp,
       ...
       AVG(Prewrite_time)*1e6 `2PC ->|PreWri`,
       AVG(Commit_time)*1e6 `2PC ->|Commit`,
