...
final class ProcessKqueue extends BaseEventProcessor<OsxProcess> {
    ...
    public void registerProcess(OsxProcess process) {
        ...
        events[0].EV_SET((long) pid, Kevent.EVFILT_PROC, Kevent.EV_ADD | Kevent.EV_RECEIPT | Kevent.EV_ONESHOT, Kevent.NOTE_EXIT | Kevent.NOTE_EXITSTATUS | Kevent.NOTE_REAP, 0l, pidPointer);
    ...
    private void processEvent(Kevent kevent) {
        ...
        } else if (filter == Kevent.EVFILT_WRITE && ident == osxProcess.getStdin().get()) // Room in stdin pipe available to write
        {
            ...
            if (!userWantsMore && stdinFd != -1) {
                ...
                events[0].EV_SET(osxProcess.getStdin().get(), Kevent.EVFILT_WRITE, Kevent.EV_DISABLE | Kevent.EV_RECEIPT, 0, 0l, Pointer.createConstant(osxProcess.getPid()));
    ...
    private void checkWaitWrites() {
        ...
        if (!processes.isEmpty()) {
            ...
            for (int i = 0; i < processes.size(); i++) {
                ...
                if (fd != -1) {
                    kevents[numKevents].EV_SET(fd, Kevent.EVFILT_WRITE, Kevent.EV_ENABLE | Kevent.EV_RECEIPT, 0, 0l, Pointer.createConstant(process.getPid()));
                    numKevents++;
                }
            }
