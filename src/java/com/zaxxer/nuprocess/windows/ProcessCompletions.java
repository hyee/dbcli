...
public final class ProcessCompletions implements Runnable {
    ...
    static {
        ...
        DEADPOOL_POLL_INTERVAL = Math.min(lingerTimeMs, Math.max(100, Integer.getInteger("com.zaxxer.nuprocess.deadPoolPollMs", 32767)));
    ...
    private void queueWrite(final WindowsProcess process) {
        ...
        if (!stdinPipe.registered) {
            HANDLE completionPort = NuKernel32.CreateIoCompletionPort(stdinPipe.pipeHandle, ioCompletionPort, new ULONG_PTR(stdinPipe.ioCompletionKey), WindowsProcess.PROCESSOR_THREADS);
            if (!ioCompletionPort.equals(completionPort)) {
                throw new RuntimeException("CreateIoCompletionPort() failed, error code: " + process.getError());
            }
        ...
        if (NuKernel32.WriteFile(stdinPipe.pipeHandle, stdinPipe.buffer, 0, null, stdinPipe.overlapped) == 0 && Native.getLastError() != NuWinNT.ERROR_IO_PENDING) {
            process.stdinClose();
        }
    }
        ...
        if (NuKernel32.ReadFile(pipe.pipeHandle, pipe.buffer, pipe.buffer.remaining(), null, pipe.overlapped) == 0) {
            ...
            switch (lastError) {
                ...
                case NuWinNT.ERROR_PIPE_NOT_CONNECTED:
                    ...
                    break;
                default:
                    System.err.println("Some other error occurred reading the pipe: " + process.getError());
                    break;
            }
        }
    }
    ...
    private void checkPendingPool() {
        ...
        while ((process = pendingPool.poll()) != null) {
            HANDLE completionPort1 = NuKernel32.CreateIoCompletionPort(process.getStdoutPipe().pipeHandle, ioCompletionPort, new ULONG_PTR(process.getStdoutPipe().ioCompletionKey), WindowsProcess.PROCESSOR_THREADS);
            if (!ioCompletionPort.equals(completionPort1)) {
                throw new RuntimeException("CreateIoCompletionPort() failed, error code: " + process.getError());
            }

            HANDLE completionPort2 = NuKernel32.CreateIoCompletionPort(process.getStderrPipe().pipeHandle, ioCompletionPort, new ULONG_PTR(process.getStderrPipe().ioCompletionKey), WindowsProcess.PROCESSOR_THREADS);
    ...
    private void initCompletionPort() {
        ...
        if (ioCompletionPort == null) {
            throw new RuntimeException("CreateIoCompletionPort() failed, error code: " + WindowsProcess.getError());
        }
    }
}
