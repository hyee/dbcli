...
public final class WindowsProcess implements NuProcess {
    ...
    public static String getError() {
        ...
        if (0 == NuKernel32.FormatMessage(0x00000100 | 0x00001000 | 0x00000200, null, Native.getLastError(), 0x409, buffer, 0, null)) {
            throw new LastErrorException(Native.getLastError());
        }
        String s = buffer.getValue().getWideString(0);
        NuKernel32.LocalFree(buffer.getValue());
        return s.trim();
    }
/*
    public void writeConsoleInput(Kernel32.INPUT_RECORD[] input) {
        IntByReference buffer=new IntByReference();
        if(!NuKernel32.WriteConsoleInput(processInfo.hProcess,input,input.length,buffer))
            throw new LastErrorException(getError());
        NuKernel32.LocalFree(buffer.getPointer());
    }*/
    ...
    public int waitForInputIdle(int milliSeconds) {
        DWORD timeout = new DWORD(milliSeconds);
        DWORD result = NuUser32.WaitForInputIdle(processInfo.hProcess, timeout);
    ...
    private void checkHandleValidity(HANDLE handle) {
        if (NuWinNT.INVALID_HANDLE_VALUE.getPointer().equals(handle)) {
            throw new RuntimeException("Unable to create pipe, error " + getError());
        }
    }

    private void checkPipeConnected(int status) {
        if (status == 0 && ((Native.getLastError()) != NuWinNT.ERROR_PIPE_CONNECTED)) {
            throw new RuntimeException("Unable to connect pipe, error: " + getError());
        }
    }
