/*[[Test the execution plan changes by adjusting the new fix controls and session environments. Usage: @@NAME <sql_id> <low_OFE> [high_OFE] [-batch"<number>"] [-ofe|-env] [-t"<secs>"] [-f"<plan_filter>"|-k"<keyword>]
   ...
   Or: @@NAME . [<options>] <SQL_Text with EOF> to test since 8i
    ...
    -accu      : test the options in accumulation mode, instead of turning on/off one by one
...
BEGIN
    ...
    DELETE sys.plan_table$ WHERE statement_id IN (
        ...
        FROM   (SELECT statement_id, id, decode(stats, lag(stats) over(ORDER BY id), 'Y', 'N') is_Delete
    ...
    dbms_lob.freetemporary(sql_text);
    ...
    OPEN :cur FOR
        ...
        finals AS
            (SELECT a.*,
             ...
             WHERE  seq <= 10)
        SELECT rownum "#", a.*
        FROM   (SELECT   --statement_id stmt_id,
                         is_matched matched,
                         plan_hash,
                         plan_hash2,
                         plan_lines lines,
                         ...
                         description || CASE WHEN a.description LIKE '%OFE_BASELINE_LOW%'  THEN low_ofe
                                             WHEN a.description LIKE '%OFE_BASELINE_HIGH%' THEN high_ofe END "Top 10 Descriptions"
                FROM   finals a,
                       table(ofeela) b
                WHERE  seq = 1
