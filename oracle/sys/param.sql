...
SELECT * FROM &GV
        ...
        FROM   x$ksppcv y 
        JOIN   x$ksppi x USING(indx)
        JOIN   (select indx,ksppstdvl sysval,row_number() over(partition by indx order by indx &con) seq_ from x$ksppsv) s USING(indx)
        LEFT   JOIN (select PNAME_QKSCESYROW ksppinm, 1 flag &con FROM X$QKSCESYS) z USING(ksppinm)
        WHERE  seq_=1 
