...
function unwrap.unwrap(obj)
    local rs=db:exec([[
        SELECT TEXT,
         ...
         FROM  DBA_SOURCE
         WHERE OWNER = upper(regexp_substr(:1,'[^\\.]+'))
    ...
    print("Result written to file "..env.write_cache(obj..'.sql',result))
end

function unwrap.onload()
    env.set_command(nil,"unwrap",'Usage: unwrap <owner>.<object_name>',unwrap.unwrap,false,2)
end
