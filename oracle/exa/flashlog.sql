...
FROM v$cell_state  a,XMLTABLE('/flashlogstore_stats' PASSING XMLTYPE(a.statistics_value) COLUMNS --
...
PRO ================
SELECT (select extractvalue(xmltype(c.confval),'/cli-output/context/@cell') from v$cell_config c where c.CELLNAME=a.CELL_NAME and rownum<2) cell,b.*
 FROM v$cell_state  a,XMLTABLE('/flashlog_stats' PASSING XMLTYPE(a.statistics_value) COLUMNS --
