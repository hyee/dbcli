...
function xplan.explain(fmt,sql)
		...
		local sn={sql=sql,res="#VARCHAR"}
		db:internal_call([[
			declare
			    sql_text clob:=:sql;
			    sn1 int:=dbms_sqltune.SQLTEXT_TO_SIGNATURE(sql_text,false); 
			    sn2 int:=dbms_sqltune.SQLTEXT_TO_SIGNATURE(sql_text,true); 
			begin 
				:res:='Extract-Signature: '||sn1||'        Force-Matching-Signature: '||sn2;
			end;]],sn)
		print(sn.res)
        local exp_sql="Explain PLAN SET STATEMENT_ID='INTERNAL_DBCLI_CMD' FOR "..sql
        if #sql>32700 then
            local args={}
            sql=sql:gsub("(:[%w_$]+)",function(s) args[s:sub(2)]=""; return s end)
		    db:internal_call(exp_sql,args)
        else
            db:internal_call('BEGIN EXECUTE IMMEDIATE :sq;END;',{sq=exp_sql})
        end
		sql=[[
