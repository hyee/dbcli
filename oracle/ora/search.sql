...
WHERE  UPPER(OWNER || '.' || OBJECT_NAME || chr(1) || OBJECT_ID || chr(1) ||
             ...
             TO_CHAR(LAST_DDL_TIME, 'YYYY-MM-DD HH24:MI:SS') || chr(1) || STATUS) LIKE '%' || NVL(UPPER(:V1), 'x') || '%' escape '\'
UNION ALL
SELECT a.owner,
       a.object_name,
...
AND    upper(a.owner||'.'||a.procedure_name || CHR(1) || a.subprogram_id|| chr(1) ||'PROCEDURE/FUNCTION'|| chr(1)) LIKE '%' || NVL(UPPER(:V1), 'x') || '%' escape '\'
...
&check_access_fn WHERE  name||' '||VERSION like '%'||UPPER(:V1)||'%' escape '\'
ORDER  BY 1, 2