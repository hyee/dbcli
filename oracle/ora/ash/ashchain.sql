/*[[
    ...
    --[[
        ...
        @exec_id:  11.2={CONNECT_BY_ROOT sql_id||nvl(sql_exec_id||to_char(sql_exec_start,'yymmddhh24miss'),session_id||','||&inst1||','||seq#) } default={CONNECT_BY_ROOT sql_id||session_id||','||&inst1||','||seq#)}
    --]]
]]*/
...
BEGIN
    IF &tree=0 THEN
        OPEN :cur FOR
            WITH bclass   AS (SELECT /*+no_merge inline*/ class, ROWNUM r FROM v$waitstat),
                 ash_base AS (SELECT /*+materialize use_hash(@GV_ASHV A@GV_ASHV) */ a.*,CASE WHEN b_sid_ LIKE '@%' THEN NULL ELSE b_sid_ END b_sid FROM &target a),
                 ash_data AS (
              ...
              WHERE  b.chose IS NOT NULL OR a.b_sid IS NOT NULL),
            chains AS (
    ...
    ELSE
        OPEN :cur FOR
            WITH bclass AS (SELECT /*+no_merge inline*/ class, ROWNUM r FROM v$waitstat),
            ash_base AS (SELECT /*+materialize*/ a.*,CASE WHEN b_sid_ LIKE '@%' THEN NULL ELSE b_sid_ END b_sid FROM &target a),
            ash_data AS (
