/*[[Show AWR Top events for a specific period. Usage: @@NAME {[0|a|<inst_id>|event_name_key|wait_class] [yymmddhh24mi] [yymmddhh24mi] [-avg] [-c]}
    ...
    -c:   compute the percentage of histogram with wait_count, instead of wait_count*slot_time
    --[[
         &avg: default={1} avg={max(secs)}
         &unit: default={slot_time} c={1}
    --]]
]]*/
...
histogram as(
       ...
       FROM   (SELECT inst,nvl(event, '- Wait Class: ' || nvl(wait_class, 'All')) event,
                     ...
                     '>=1m'
                     END unit,
                     nullif(round(ratio_to_report(SUM(c * flag * &unit)) OVER(PARTITION BY inst,wait_class,event) * 100, 2), 0) pct
...
SELECT inst, '- * ON CPU *' event,null wait_class,max(cpu_count) counts,null timeouts,sum(value*flag)* 1e-6/&avg waited,
       ...
       null "<1us",null "<2us",null "<4us",null "<8us",null "<16us",null "<32us",null "<64us",null "<128us",null "<256us",null "<512us",null "<1ms",null "<2ms",null "<4ms",null "<8ms",null "<16ms",null "<33ms",null "<66ms",null "<131ms",null "<262ms",null "<524ms",null "<1s",null "<2s",null "<4s",null "<8s",null "<17s",null "<34s",null "<1m",null ">=1m"
from   time_model a 
