/*[[
    ...
    -avg: compute per second, instead of total
    -c:   compute the percentage of histogram with wait_count, instead of wait_count*log(2,slot_time)
    -d:   when an event or wait class is specified, use this option to group data by date instead of by each snapshot
    ...
    OPEN :c FOR
        ...
        time_model AS(
            SELECT dbid,snap_id,pkey,instance_number,inst,secs,etime,adj,cpu_count,
                   ...
                   &avg div
            FROM  (SELECT s.*,p.value cpu_count
        ...
        event AS(
            SELECT grouping_id(inst,wait_class,event) grp,nvl(inst,'*') inst,
                   ...
                   round(sum(waits/div/snaps),&rd) counts,
                   nullif(round(sum(timeouts)/nullif(sum(waits),0),4),0) timeouts,
                   round(sum(micro/div/snaps),&rd) waited,
                   round(sum(micro)/nullif(sum(DISTINCT ela),0),4) db,
                   round(sum(micro)/nullif(sum(waits),0),2) avg_wait
            FROM  (SELECT event_name event,wait_class,div,decode(f,0,''||inst,to_char(etime,'&fmt')) inst,
                          ...
                          sum(DISTINCT ela) OVER(PARTITION BY snap_id,inst) ela,
                          count(DISTINCT snap_id) OVER(PARTITION BY inst) snaps
                   FROM   time_model
