/*[[
    Compare SQL performance by different snapshot ranges. Usage: type `help @@NAME` for more details.
    ...
    * @@NAME -dbid <dbid1,yymmddhh24mi,yymmddhh24mi> <dbid2,yymmddhh24mi,yymmddhh24mi> : compare awr snapshots between two dbids
    ...
    Other options:
    ==============
    -f"<filter>"       : additional filter on dba_hist_sqlstat/gv$sqlstats
    -same              : exclude same plans that exist in both `PRE` and `POST`
...
col hv,sig_weight,sig_rows,plans,grps,all_ela,all_execs,seq,dbid noprint
col signature,# break
set autohide col
...
BEGIN
    ...
    WITH r AS(
        ...
        FROM   (SELECT dbid,
                       CASE WHEN dbid=dbid1 AND end_interval_time+0 BETWEEN st1-5/1440 AND ed1+5/1440 THEN 'PRE' ELSE 'POST' END grp,
                ...
                FROM (SELECT a.*,
                      ...
                      AND    (dbid=dbid1 AND end_interval_time+0 BETWEEN st1-5/1440 AND ed1+5/1440 OR
                             dbid=dbid2 AND end_interval_time+0 BETWEEN st2-5/1440 AND ed2+5/1440)
                      AND    (&filter)
                      AND    ('&instance' IS NULL OR instance_number=0+'&instance')) a
                GROUP  BY dbid,plan_hash_value,sql_id,force_matching_signature,
                          CASE WHEN dbid=dbid1 AND end_interval_time+0 BETWEEN st1-5/1440 AND ed1+5/1440 THEN 'PRE' ELSE 'POST' END
                HAVING sum(exec) > 0)
    ...
    r1 AS(
        SELECT sum(decode(grp,'POST',all_ela/all_execs)) OVER(PARTITION BY &sql)/
               ...
               max(decode(grp,'POST',avg_ela*execs)) OVER(PARTITION BY &sql) sig_weight,
               count(*) OVER(PARTITION BY &sql) sig_rows
        FROM   (SELECT r.*,
    ...
    r2 AS(
        ...
        FROM (
            SELECT dense_rank() OVER(ORDER BY sig_weight DESC,avg_diff,hv) "#",
                   sig_weight/nullif(sum(sig_weight/sig_rows) OVER(),0) "Weight",
                   r1.*
            FROM   r1
            WHERE  (avg_diff &diff)
