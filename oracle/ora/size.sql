/*[[
    ...
    If no segment name is specified, then the top 100 segments will be listed.
    This script needs the 'SELECT ANY DICTIONARY' privilege.

    -d: Show the details at segment level instead of object level; only the top 1000 segments are listed
    -a: Include all schemas, by default only the current schema is included.
    ...
    --[[
        ...
        &OPT3: default={null}, d={subobject_name}
...
BEGIN
    IF :v1 IS NOT NULL THEN
        ...
        tab  AS(SELECT * FROM clu
                UNION ALL
                SELECT /*+ordered use_nl(c t)*/ lv+1, t.obj#, c.subname
                FROM   clu c, sys.tab$ t
                WHERE  t.bobj# = c.obj#),
        ind  AS(SELECT * FROM tab
                UNION ALL
                SELECT /*+ordered use_nl(c t o)*/ lv+1, t.obj#, c.subname
                FROM   tab c, sys.ind$ t
                WHERE  t.bo# = c.obj#),
        lobs AS(SELECT * FROM ind
                UNION
                SELECT /*+ordered use_nl(c t o)*/ lv+1, t.lobj#, c.subname
                FROM   ind c, sys.lob$ t
                WHERE  t.obj# = c.obj#),
        rws  AS(SELECT /*+materialize no_expand*/ a.*, inserts-deletes rws
                FROM   sys.dba_tab_modifications a
                WHERE  table_owner = :object_owner
                AND    table_name = :object_name
                AND    (:object_subname IS NULL OR :object_subname IN (partition_name,subpartition_name)))
        SELECT  /*+ordered use_hash(a b) use_nl(o2) no_parallel*/ a.*,
                CASE WHEN object_type LIKE 'TABLE%' THEN
                    nvl(coalesce((SELECT rowcnt FROM sys.tab$ WHERE obj# = o2.obj#),
                                 (SELECT rowcnt FROM sys.tabpart$ WHERE obj# = o2.obj#),
                                 (SELECT rowcnt FROM sys.tabsubpart$ WHERE obj# = o2.obj#)),0) + nvl(b.rws,0)
                END num_rows
        FROM(
        ...
        LEFT JOIN sys.obj$ o2
        ON   (o2.owner# = a.user_id
              AND o2.name = a.object_name
              AND nvl(o2.subname,' ') = nvl(a.partition_name,' ')
              AND o2.namespace = 1)
        LEFT JOIN rws b
        ON   (a.owner = b.table_owner
              AND a.object_name = b.table_name
              AND (a.partition_name = b.subpartition_name OR nvl(a.partition_name,' ') = nvl(b.partition_name,' ') AND b.subpartition_name IS NULL)
        ...
        $IF dbms_db_version.version > 10 $THEN
            ...
            FOR r IN (SELECT extractvalue(column_value,'//OWNER') owner,
                ...
                ELSIF r.object_type LIKE 'TABLE%' THEN
                    FOR r1 IN(
                            SELECT tablespace_name,avg_row_len,num_rows,pct_free,nvl(regexp_substr(degree,'\d+'),'1')+0 degree,ini_trans
                            FROM   dba_tables
                            ...
                            UNION ALL
                            SELECT tablespace_name,avg_row_len,num_rows,pct_free,null degree,ini_trans
                            ...
                            AND    avg_row_len > 0
                            AND    num_rows > 0
                            AND    r.object_type = 'TABLE PARTITION'
                            UNION ALL
                            SELECT tablespace_name,avg_row_len,num_rows,pct_free,null degree,ini_trans
                            FROM   dba_tab_subpartitions
                            WHERE  table_name = r.object_name
                            AND    table_owner = r.owner
                            AND    subpartition_name = r.partition_name
                            AND    avg_row_len > 0
                            AND    num_rows > 0
                            AND    r.object_type = 'TABLE SUBPARTITION') LOOP
