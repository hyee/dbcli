...
BEGIN
    ...
    ELSE
        ...
        qry := q'!SELECT 'Event' typ,
                       event_name NAME,
                       SUM(TIME_WAITED_MICRO * DECODE(snap_id, mid, -1, 1)/decode('&div','1',1,secs)) micro,
                       SUM(TOTAL_WAITS * DECODE(snap_id, mid, -1, 1)/decode('&div','1',1,secs)) cnt
                FROM   snap a
                ...
                SELECT 'Latch' typ,
                       latch_name,
                       SUM(WAIT_TIME * DECODE(snap_id, mid, -1, 1)/decode('&div','1',1,secs)) micro,
                       SUM(GETS * DECODE(snap_id, mid, -1, 1)/decode('&div','1',1,secs)) cnt
                FROM   snap a
                JOIN   &flag.latch b
                USING  (dbid, instance_number)
                WHERE  dbid = &did --
                ...
                GROUP  BY latch_name
                UNION ALL
                SELECT 'Stat' typ, stat_name, SUM(VALUE * DECODE(snap_id, mid, -1, 1)/decode('&div','1',1,secs)) micro, NULL cnt
                FROM   snap a
                JOIN   &flag.sysstat b
                USING  (dbid, instance_number)
                WHERE  dbid = &did --
    ...
    qry:=replace(replace(q'!
        ...
        STATS AS(
            ...
            RULES SEQUENTIAL ORDER(
                    cnt['redo size']=SUM(micro)[NAME LIKE 'redo write size count%'],
                    ...
                    delta_time['redo write finish time']=avg_time['redo write finish time']-avg_time['redo write issue time'],
