	   ...
	   SELECT INDX,
               ...
               CASE
                   ...
                   WHEN CACHE_NAME IN ('dc_segments') THEN
                    'SYS.SEG$: file#' || to_number(substr(key, 11, 4), 'XXXX') || 
                            ' block#' || to_number(utl_raw.reverse(substr(key, 17, 6)), 'XXXXXX')
               END object_name,
               num,
               SUBSTR(KEY, 1, 80) KEY
        FROM   (SELECT a.*, to_number(UTL_RAW.REVERSE(SUBSTR(KEY, 1, 8)), 'XXXXXXXX') num FROM v$rowcache_parent a))))
WHERE  :V1 IS NOT NULL AND (upper(object_name) LIKE upper('%&V1%') OR cache_name = :V1) OR :V1 IS NULL AND LOCK_MODE+LOCK_REQUEST>0