/*[[
    Oracle Performance Hub. Usage: @@NAME [{[1|YYMMDDHH24MI] [YYMMDDHH24MI] [<sql_id>|<component>]} | {[sid] [inst_id] [serial#]}] [-snap"<minutes>"]
      ...
      <sid> [<inst>]          : Generate realtime(gv$*) report on specific session, start time defaults to 3 hours ago if "-snap" is not specified
      <sid> [<inst>] <serial#>: Generate non-realtime(dba_hist*) report on specific session
      <component>             : can be combination with timepicker/summary/rac/ash/workload/monitor/addm/exa/cell concatenated by '+'. e.g: +exa+addm

    The report is saved as <cache>/perfhub_<start>_<end>.html for a time range,
    or as <cache>/sidhub_<sid>_<serial>_<inst>.html when a session is given.
    <start> and <end> are YYMMDDHH24MI timestamps.
    --[[
...
BEGIN
    BEGIN
        v_start  := coalesce(to_date(nullif(:v1, '1'), 'YYMMDDHH24MI'), SYSDATE - CASE WHEN v_realtime = 1 THEN nvl(&snap, 3/24) ELSE 7 END);
        ...
        ELSIF :v3 IS NULL
              OR instr(:v3, '+') > 0
              OR instr(:v3, '-') > 0
              OR lower(:v3) IN ('timepicker', 'summary', 'rac', 'ash', 'workload', 'monitor', 'addm', 'exa') THEN
