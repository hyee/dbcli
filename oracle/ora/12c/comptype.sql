/*[[
    Get the compression type of the blocks that a table stores. Usage: @@NAME [<owner>.]<table_name>[.<partition_name>] [<rows>|-all] [-f"<filter>"] [-dx|-sample]

    Parameters:
        rows   : The maximum number of rows to scan, default as 2 millions. Use '-all' to scan without a limit.
        -f     : Scan only the rows that match the filter expression.
        -dx    : Read the blocks with direct path reads, instead of reading them through the buffer cache.
        -sample: Use a BLOCK SAMPLE sized from the table statistics, instead of scanning an exact number of rows. It falls back to the exact limit when the statistics are missing, or when the sample would cover more than 80% of the blocks.

    Both -dx and -f run the scan in parallel(4). Both -f and -sample skip the extent map, so their BLOCKS column counts the blocks that hold the scanned rows, instead of the blocks those rows occupy.
    ...
    For DBIM, the CU size is 32MB and a CU can hold more than 50,000 rows

    Use "alter session set events 'trace[ADVCMP_COMP] disk=lowest'" and move the table to see the detailed compression ratio

    Sample Output:
    ==============
    SQL> @@NAME ssb.lineorder 10000000
    OBJECT_ID DATA_OBJECT_ID PARTITION_NAME COMPRESSION_TYPE HEADER_BLOCKS BLOCKS UNKNOWNS       ROWS Rows/Block
    --------- -------------- -------------- ---------------- ------------- ------ -------- ---------- ----------
    ...
    --[[
        ...
        &px    : default={no_parallel} dx={parallel(4)} f={parallel(4)}
...
BEGIN
    ...
    v_stmt := q'[
        ...
        OBJS AS(SELECT /*+materialize*/ object_id, data_object_id, nvl(subobject_name, ' ') pname
                ...
                AND    object_name = '&object_name' @PART1@),
        EXTS AS(&extent)
        SELECT * FROM (
            ...
            ON     (b.pname = e.pname
                    AND e.f = a.f
                    AND e.grp = trunc(a.blk / greatest(1, trunc(8 * 2E6 / @TOP@)) / 8192)
                    AND a.blk BETWEEN e.bid AND e.eid)
