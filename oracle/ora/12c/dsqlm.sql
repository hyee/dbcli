/*[[Get SQL Monitor reports from dba_hist_reports, supports 12c only. Usage: @@NAME {[sql_id|report_id] [YYYYMMDDHH24MI] [YYYYMMDDHH24MI]} [-f"<filter>"] [-avg]

  <sql_id>       : List the SQL Monitor records of that SQL_ID, one row per report
  <report_id>    : A number is a report_id, it dumps that saved SQL Monitor report
  [start] [end]  : Restrict the search window, format YYYYMMDDHH24MI, defaults to
                   the last 7 days up to now
  -sql"<keyword>": List the records whose SQL text contains <keyword>
  -u["user"]     : Only show the SQL list of the current schema, or of "user"
  -hub           : Generate the SQL Performance Hub report for the given date range
  -avg           : Show average cost instead of total cost
  -f"<filter>"   : An extra predicate on the report rows, e.g. -f"username='SYS'"
  -t"<table>"    : The data source, defaults to sys.dba_hist_reports

  The dumped report is written to sqlm_<sql_id>_<report_id>.html in the format
  below (the extension is .html even for -text). Only one of them takes effect.
  Format:
    -active : output file is in active HTML format (the default)
  ...
  --[[
    @ver   : 12.1={}
    &filt  : default={KEY1=nvl(v_sql_id,KEY1)}, sql={lower(extractvalue(xmltype(a.report_summary), '//sql_text')) like lower('%&0%')}, u={extractvalue(xmltype(a.report_summary), '//user') = nvl('&0', sys_context('userenv', 'current_schema'))}
    ...
    &out   : default={active} active={active} html={html} em={em} text={text}
  --]]
]]*/
