/*[[
    Show the per-event wait time histogram, as the delta between two samples. Usage: @@NAME {<0|secs> [0|inst_id]} [-lgwr|-io|-gc|-w"<event|wait_class>"|-f"<filter>"] [-c|-n]

    Parameters:
        <secs>  : the interval, in seconds, to sample the stats in order to calculate the delta values. When 0, the cumulative values since instance startup are reported instead of delta values, and the Time/s, Waits/s columns degrade to cumulative totals. Default to 10
        inst_id : the instance to sample. 0 or omitted samples all instances
        -lgwr   : show only the events relative to LGWR
        -io     : show only the events belonging to the User I/O and System I/O wait classes
        -gc     : show only the events belonging to the Cluster wait class or starting with gc
        -w      : show only the events whose event name or wait class matches the given name
        -f      : show only the events matching the given SQL predicate on v$system_event
        -c      : base the percentages of the histogram on the wait count, instead of the wait time
        -n      : show the weighted values of the histogram, instead of the percentages
    ...
    --[[
        ...
        &hn: default={the percentages of [wait_count&u]} n={the weighted values of [wait_count&u]}
...
col Waits/s for k1
col g noprint
pro Sampling stats for &V1 seconds, the values of histogram are &hn:
pro =============================================================================================================
