/*[[
    View/Create/Modify SQL PLAN Baselines (SPM). Usage: @@NAME [load|drop|enable|disable|fix|unfix <keyword> [<phv>]] | [<keyword>|-f"<filter>"]  
...
DECLARE
    ...
    BEGIN
        dbms_output.put_line(utl_lms.format_message('SPM is %s%s (SQL_ID = %s / PLAN_HASH_VALUE = %s / SQL_HANDLE = %s / PLAN_NAME = %s).',
                        CASE WHEN done=0 THEN 'not ' END,
                        replace(op,'eed','ed'),
                        regexp_substr(plan_name,'PLAN_(.{13})',1,1,'i',1),
                        ''||to_number(regexp_substr(plan_name,'(.{8})$'),'fmxxxxxxxx'),
...
BEGIN
    ...
    IF V1 IN('DROP','FIX','UNFIX','ENABLE','DISABLE','LOAD','VIEW','ACCEPT','ENVOLVE') AND V2 IS NULL THEN
        raise_application_error(-20001,'Please input more parameters to specify the target.');
    END IF;
    IF V1 IN('FIX','UNFIX','ENABLE','DISABLE') THEN
        tmp_now := SYSDATE;
            ...
            ELSIF NOT phvs.exists(''||r.phv) THEN
                IF r.grp = 'cursor' THEN
                    tmp := dbms_spm.load_plans_from_cursor_cache(sql_id=>r.sql_id,
                                                                 ...
                                                                 fixed=>'NO',
                                                                 enabled=>'YES');
                $IF DBMS_DB_VERSION.RELEASE+DBMS_DB_VERSION.VERSION > 13 $THEN
        ...
        OPEN c FOR
            SELECT * FROM (
                SELECT regexp_replace(plan_name,'PLAN_(.{13})','PLAN_$PROMPTCOLOR$\1$NOR$') "PLAN_NAME (SQL_Id:13,PHV:8)",
                       to_number(regexp_substr(plan_name,'(.{8})$'),'fmxxxxxxxx') plan_hash,
                       signature,
                ...
                AND    (tmp_now IS NULL OR greatest(created+0,
                                  nvl(last_modified+0,sysdate-3650),
                                  nvl(last_executed+0,sysdate-3650),
                                  nvl(last_verified+0,sysdate-3650))>=tmp_now)
                ORDER BY greatest(created+0,
                                  nvl(last_modified+0,sysdate-3650),
                                  nvl(last_executed+0,sysdate-3650),
                                  nvl(last_verified+0,sysdate-3650)) DESC NULLS LAST)
            WHERE ROWNUM<=50;
    END IF;
    :c := c;
END;
