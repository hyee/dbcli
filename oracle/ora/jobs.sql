...
BEGIN
    IF :object_id IS NOT NULL THEN
        IF :object_type != 'JOB' THEN
            raise_application_error(-20001,'Target object is not a job');
        END IF;
        :flag := '\G';
    ELSE
        :flag := ' ';
...
where (owner=nvl(upper(:V1),sys_context('userenv','current_schema')) or upper(:V1)='ALL' or :object_name=job_name)
and   (nvl(NEXT_RUN_DATE,date'4000-1-1') != date'4000-1-1' or state='RUNNING') &flag;
...
select * from(
    ...
    where (owner=nvl(upper(:V1),sys_context('userenv','current_schema')) or upper(:V1)='ALL'  or :object_name is not null and job_name like :object_name||'%')
    and   (&filter)
    order by 2 desc)
where rownum<=50;