...
DECLARE
    ...
    BEGIN
        ...
        IF NOT v_founds.exists(obj) /*OR '&F1'!='p_obj#'*/ THEN
            ...
            AND    &F1 !=obj
            UNION ALL
            SELECT  /*+index(a)*/DECODE('&F1','p_obj#',nvl(robj#,obj#),obj#),level lv, &F3
            FROM   sys.cdef$ a
		...
		SELECT --+ordered use_nl(dep op us) index(us) no_expand
			   LPAD(' ',lv*2*&CC) ||us.name||'.'||op.name
				  ...
				  decode(op.type#,
					  ...
					  op.ctime CREATED,
					  op.mtime LAST_DDL
		FROM   dep,sys.obj$ op,sys.user$ us
		WHERE  dep.obj = op.obj#
		AND    op.owner#=us.user#
		AND   (us.name NOT IN('SYS','SYSTEM','PUBLIC') or v_owner in('SYS','SYSTEM','PUBLIC')) 
        AND   (op.type#!=5 or us.name in('SYS','SYSTEM','PUBLIC')))
