/*[[Show ash cost for a specific SQL. usage: ashplan <sql_id> [[plan_hash_value|a] [YYMMDDHH24MI] [YYMMDDHH24MI]]
--[[
  Templates:
...
WITH sql_plan_data AS
 (SELECT *
  FROM   (SELECT a.*,
                 dense_rank() OVER(ORDER BY flag, tm DESC, child_number DESC, plan_hash_value DESC) seq
          FROM   (SELECT id,
                         parent_id,
                         child_number    ha,
                         1               flag,
...
ordered_hierarchy_data AS
 (SELECT id,
  ...
  FROM   hierarchy_data),
qry AS
 (SELECT /*+materialize*/ DISTINCT sql_id sq,
                  flag flag,
