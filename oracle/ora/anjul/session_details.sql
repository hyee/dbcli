...
DECLARE
    ...
    ----------------------------------------------------------
    FUNCTION appdba_format_duration(p_seconds NUMBER) RETURN VARCHAR2 IS
    ...
    --    kb, mb or gb
    ----------------------------------------------------------
    FUNCTION appdba_format_size(p_bytes NUMBER) RETURN VARCHAR2 IS
    ...
    ---------------------------------------------------
    PROCEDURE print_sql_statement(p_hash_value gv$sqltext.hash_value%TYPE, p_inst_id gv$sqltext.inst_id%TYPE) IS
...
BEGIN
    ...
    v_sid     := NVL(:V1,USERENV('SID'));
    v_inst_id := NVL(:V2, USERENV('INSTANCE'));
    ...
    ---------------------------------------------------
    DBMS_OUTPUT.PUT_LINE('$HEADCOLOR$Now         $NOR$: ' || TO_CHAR(SYSDATE, 'Dy DD-MON-YYYY HH24:MI:SS'));
    DBMS_OUTPUT.PUT_LINE('$HEADCOLOR$SID/Serial  $NOR$: ' || sess_details.sid || ',' || sess_details.serial#);
    DBMS_OUTPUT.PUT_LINE('$HEADCOLOR$Instance ID $NOR$: ' || sess_details.inst_id);
    DBMS_OUTPUT.PUT_LINE('$HEADCOLOR$Foreground  $NOR$: ' || 'PID: ' || sess_details.process || ' - ' || sess_details.program);
    DBMS_OUTPUT.PUT_LINE('$HEADCOLOR$Shadow      $NOR$: ' || 'PID: ' || proc_details.spid || ' - ' || proc_details.program);
    FOR c1 IN (SELECT network_service_banner
        ...
        DBMS_OUTPUT.PUT_LINE('$HEADCOLOR$Protocol    $NOR$: ' || c1.network_service_banner);
    END LOOP;
    DBMS_OUTPUT.PUT_LINE('$HEADCOLOR$OS User     $NOR$: ' || sess_details.osuser || ' on ' || sess_details.machine || ' (' || sess_details.module || ')');
    DBMS_OUTPUT.PUT_LINE('$HEADCOLOR$Ora User    $NOR$: ' || sess_details.username);
    DBMS_OUTPUT.PUT_LINE('$HEADCOLOR$Status Flags$NOR$: ' || sess_details.status || ' ' || sess_details.server || ' ' || sess_details.type);
    DBMS_OUTPUT.PUT_LINE('$HEADCOLOR$Login Time  $NOR$: ' || TO_CHAR(sess_details.logon_time, 'Dy DD-MON-YYYY HH24:MI:SS') || ' - ' ||
                         appdba_format_duration((SYSDATE - sess_details.logon_time) * 60 * 60 * 24));
    DBMS_OUTPUT.PUT_LINE('$HEADCOLOR$Last Call   $NOR$: ' || TO_CHAR(SYSDATE - (sess_details.last_call_et / 60 / 60 / 24), 'Dy DD-MON-YYYY HH24:MI:SS') || ' - ' ||
                         appdba_format_duration(sess_details.last_call_et));

    ---------------------------------------------------
    -- SQL Statements
    ---------------------------------------------------
    DBMS_OUTPUT.PUT_LINE('$HEADCOLOR$Current SQL $NOR$: ' || RPAD('Address ''' || sess_details.sql_address || ''' ', 19) || 'Hash Value ''' ||
                         sess_details.sql_hash_value || '''');
    print_sql_statement(sess_details.sql_hash_value, sess_details.inst_id);

    DBMS_OUTPUT.PUT_LINE('$HEADCOLOR$Previous SQL$NOR$: ' || RPAD('Address ''' || sess_details.prev_sql_addr || ''' ', 19) || 'Hash Value ''' ||
                         sess_details.prev_hash_value || '''');
    print_sql_statement(sess_details.prev_hash_value, sess_details.inst_id);

    ---------------------------------------------------
    -- Wait Details
    ---------------------------------------------------
    DBMS_OUTPUT.PUT_LINE('$HEADCOLOR$Session Wait$NOR$:');
    FOR c1 IN (SELECT w.state, w.event, w.p1, w.p2, w.p3, n.parameter1 p1n, n.parameter2 p2n, n.parameter3 p3n, w.seconds_in_wait secs
               FROM   gv$session_wait w, gv$event_name n
        ...
        IF c1.state <> 'WAITING' THEN
            ...
            DBMS_OUTPUT.PUT_LINE(CHR(9) || c1.state || ': ' || c1.event || ': ' || appdba_format_duration(c1.secs));
        ELSE
            DBMS_OUTPUT.PUT_LINE(CHR(9) || 'Currently Waiting: ' || c1.event || ': ' || appdba_format_duration(c1.secs));
        END IF;
        ...
        IF c1.p1n = 'file#' AND c1.p2n = 'block#' THEN
            ...
            --
            DBMS_OUTPUT.PUT_LINE(CHR(9) || CHR(9) || RPAD(LOWER(v_segment_type) || ':', 17) || v_owner || '.' || v_segment_name);
        ...
        ELSE
            DBMS_OUTPUT.PUT_LINE(CHR(9) || CHR(9) || '(' || c1.p1n || ':' || c1.p1 || ' ' || c1.p2n || ':' || c1.p2 || ' ' || c1.p3n || ':' || c1.p3 || ')');
        END IF;
    END LOOP;
    ...
    ---------------------------------------------------
    DBMS_OUTPUT.PUT_LINE('$HEADCOLOR$Locks       $NOR$:');
    FOR c1 IN (SELECT l.type,
        ...
        ELSIF c1.type = 'RW' THEN
            v_object_name := 'FILE#=' || SUBSTR(c1.id1, 1, 3) || ' BLOCK#=' || SUBSTR(c1.id1, 4, 5) || ' ROW=' || c1.id2;
        ...
        ELSIF c1.type = 'TX' THEN
            ...
            IF c1.request > 1 THEN
                BEGIN
                    SELECT o.object_name || ' ''' || dbms_rowid.rowid_create(1,
                                                                             o.data_object_id,
                ...
                END;
            END IF;
            SELECT v_object_name || NAME || ', Slot ' || TO_CHAR(BITAND(c1.id1, TO_NUMBER('ffff', 'xxxx')) + 0) || ', Seq ' || c1.id2
            INTO   v_object_name
            FROM   v$rollname
        ...
        IF c1.request > 1 THEN
            FOR c2 IN (SELECT *
                ...
                FOR c3 IN (SELECT *
                    ...
                    DBMS_OUTPUT.PUT(CHR(9) || CHR(9) || ' ');
                    DBMS_OUTPUT.PUT_LINE('Foreground  : PID: ' || c3.process || ' - ' || c3.program);
                    DBMS_OUTPUT.PUT(CHR(9) || CHR(9) || ' ');
                    DBMS_OUTPUT.PUT_LINE('User        : ' || c3.username || ' (' || c3.osuser || ' on ' || c3.machine || ')');
                    DBMS_OUTPUT.PUT(CHR(9) || CHR(9) || ' ');
                    DBMS_OUTPUT.PUT_LINE('Status      : ' || c3.status);
                    DBMS_OUTPUT.PUT(CHR(9) || CHR(9) || ' ');
                    DBMS_OUTPUT.PUT_LINE('Login Time  : ' || to_char(c3.logon_time, 'Dy DD-MON-YYYY HH24:MI:SS') || ' - ' ||
                                         appdba_format_duration((SYSDATE - c3.logon_time) * 60 * 60 * 24));
                
                    DBMS_OUTPUT.PUT(CHR(9) || CHR(9) || ' ');
                    DBMS_OUTPUT.PUT_LINE('Last Call   : ' || to_char(SYSDATE - (c3.last_call_et / 60 / 60 / 24), 'Dy DD-MON-YYYY HH24:MI:SS') ||
    ...
    -- Long Operations
    ---------------------------------------------------
    DBMS_OUTPUT.PUT_LINE('$HEADCOLOR$Long Ops    $NOR$:');
    FOR c1 IN (SELECT lo.*
        ...
        DBMS_OUTPUT.PUT_LINE(CHR(9) || c1.MESSAGE || ': ' || appdba_format_duration(c1.time_remaining) || ' remaining (' ||
    ...
    ----------------------------------------------------
    DBMS_OUTPUT.PUT_LINE('$HEADCOLOR$Transaction $NOR$:');
    ...
    DBMS_OUTPUT.PUT_LINE('$HEADCOLOR$TEMP Usage  $NOR$:');
    FOR c1 IN (SELECT sort.blocks * ts.block_size total_bytes, DECODE(sort.segtype, 'DATA', 'DATA (Global Temp Table)', sort.segtype) segtype, extents
               FROM   gv$sort_usage sort, dba_tablespaces ts
    ...
    DBMS_OUTPUT.PUT_LINE('$HEADCOLOR$Statistics  $NOR$:');
    ...
    FOR c1 IN (SELECT event, waits, AVG, secs, pct
               FROM   (SELECT event, waits, AVG AVG, secs secs, (RATIO_TO_REPORT(secs) OVER()) * 100 pct
                       FROM   (SELECT e.event, e.total_waits waits, e.time_waited / e.total_waits / 100 AVG, e.time_waited / 100 secs
                               FROM   gv$session_event e
                               WHERE  e.sid = sess_details.sid
                               AND    e.inst_id = sess_details.inst_id
                               UNION ALL
                               SELECT n1.name event, s2.value waits, s2.value / GREATEST(s1.value, 1) / 100 AVG, s1.value / 100 secs
                               FROM   gv$statname n1, gv$sesstat s1, gv$statname n2, gv$sesstat s2
                               ...
                               ORDER  BY secs DESC))
               WHERE  pct > 0.1) LOOP
        DBMS_OUTPUT.PUT_LINE(CHR(9) || RPAD(c1.event, 35) || LPAD(NVL(TO_CHAR(c1.waits), '-'), 11) || LPAD(NVL(TO_CHAR(c1.avg, '9990.999'), '-'), 9) ||
