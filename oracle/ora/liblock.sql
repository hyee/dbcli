/*[[
Check the holder of library cache objects. Usage: @@NAME {[<sid>|<object_name>] [inst_id]} [-u]
...
lp AS(SELECT /*+materialize no_expand ordered no_merge(d) use_hash(l h d)*/DISTINCT 
      ...
      FROM  (&CHECK_ACCESS l JOIN sess h ON (l.HOLDING_USER_SESSION = h.saddr and (l.inst_id is null or l.inst_id=h.inst_id))) 
