...
SELECT grp,
       ...
       NULLIF(AVG(IOMBPS_THROTTLE_EXEMPT*1024*1024)/NULLIF(AVG(IOPS),0),0) "MB|IO",
       '|' "|",
       NULLIF(ROUND(AVG(AVG_ACTIVE_PARALLEL_STMTS),2),0) "PX|STMTS",
       NULLIF(ROUND(AVG(AVG_QUEUED_PARALLEL_STMTS),2),0) "PX|QUEUE",
       NULLIF(ROUND(AVG(AVG_ACTIVE_PARALLEL_SERVERS),2),0) "PX|SERVER",
       NULLIF(ROUND(AVG(PARALLEL_SERVERS_LIMIT),2),0) "PX|SVQUEUE",
       MAX(PARALLEL_SERVERS_LIMIT) KEEP(DENSE_RANK LAST ORDER BY END_TIME) "PX|SVLIMIT",
       '|' "|",
...
GROUP BY grp,inst_id,con_id,a.name
