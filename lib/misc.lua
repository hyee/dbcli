...
function string.chars(s,start)
    return utf8.len(s,start)
end

--Display columns s occupies, and the bytes of the line they came from. Control
--sequences, combining marks and control characters count 0 columns but still
--count bytes; East Asian wide and fullwidth count 2.
--For cross-lines strings, returns the widest columns and bytes
--Returns 2 value, terminal display columns and bytes
--A tie goes to the first line that reached the widest width. LF is the only line
--separator: CR folds the cursor back inside the line, so its byte still belongs
--to it ("abc\r\n" is one 4-byte line of 3 columns).
local ansi_width=utf8.ansi_width
...
--Cuts s to at most maxlen display columns and returns bytes, terminal display columns, cut.
--print_len is the width cut actually has, not the requested maxlen: a wide character
--straddling the boundary is dropped whole rather than half-drawn.
local ansi_cut=utf8.ansi_cut
function string.ansi_cut(s,maxlen)
    if s=="" then return 0,0,s end
    if s==nil then return nil end
    if maxlen==0 then return 0,0,'' end
    if type(s)~='string' then s=tostring(s) end
    return ansi_cut(s,maxlen)
end
